...
namespace octotiger {
    namespace reference_implementation {
        ...
        template <class U>
        U B_p(std::int64_t const opts_problem, double const physcon_c, U rho,
            U e, U mmw)
        {
            ...
            throw formatted_exception(
                "error: marshak is the only supported problem type\n");
        }

        template <class U>
        U kappa_p(std::int64_t const opts_problem, U rho, U e, U mmw, double X,
            double Z)
        {
            if (opts_problem == MARSHAK)
            {
        ...
        template <class U>
        U kappa_R(std::int64_t const opts_problem, U rho, U e, U mmw, double X,
            double Z)
        {
            if (opts_problem == MARSHAK)
            {
        ...
        template <typename F>
        void abort_if_solver_not_converged(double const eg_t0, double E0,
            ...
            for (std::int64_t i = RAD_BW; i != RAD_NX - RAD_BW; ++i)
            {
                for (std::int64_t j = RAD_BW; j != RAD_NX - RAD_BW; ++j)
                {
                    for (std::int64_t k = RAD_BW; k != RAD_NX - RAD_BW; ++k)
                    {
                        ...
                        auto const ddt = implicit_radiation_step(opts_problem,
                            physcon_c, E1, e1, F1, u1, den, mmw[iiir],
                            X_spc[iiir], Z_spc[iiir], dt);
    ...
    void radiation_cpu_kernel::operator()(std::int64_t const opts_eos,
        std::int64_t const opts_problem, double const opts_dual_energy_sw1,
        double const opts_dual_energy_sw2, double const physcon_A,
        ...
        double const clightinv)
    {
        reference_implementation::radiation_impl(opts_eos, opts_problem,
            opts_dual_energy_sw1, opts_dual_energy_sw2, physcon_A, physcon_B,
            physcon_c, er_i, fx_i, fy_i, fz_i, d, rho, sx, sy, sz, egas, tau,
