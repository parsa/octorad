...
namespace radiation_v2 {
        ...
        double const physcon_A, double const physcon_B, double d)
    {
        ...
        return (x < 0.01) ?
            4.0 * physcon_A / physcon_B * square(x) :
            8.0 * physcon_A / physcon_B * (std::sqrt(square(x) + 1.0) - 1.0);
    }
        ...
        double const Z, double const dt)
    {
        ...
        auto const B = [physcon_c, rho, c, rhoc2](double const e) {
            return (4.0 * M_PI / c) * B_p(physcon_c, rho, e * rhoc2) / rhoc2;
        };
    ...
    void radiation_impl(std::int64_t const opts_eos,
        double const opts_dual_energy_sw1, double const opts_dual_energy_sw2,
        double const physcon_A, double const physcon_B, double const physcon_c,
        ...
        for (std::int64_t i = RAD_BW; i != RAD_NX - RAD_BW; ++i)
        {
            for (std::int64_t j = RAD_BW; j != RAD_NX - RAD_BW; ++j)
            {
                for (std::int64_t k = RAD_BW; k != RAD_NX - RAD_BW; ++k)
                {
                    ...
                    auto const ddt = implicit_radiation_step(physcon_c, E1, e1,
...
{
    radiation_v2::radiation_impl(opts_eos, opts_dual_energy_sw1,
        opts_dual_energy_sw2, physcon_A, physcon_B, physcon_c, er_i, fx_i, fy_i,
        fz_i, d, rho, sx, sy, sz, egas, tau, fgamma, U, mmw, X_spc, Z_spc, dt,
        clightinv);
}
