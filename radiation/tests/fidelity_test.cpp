...
bool check_case(std::size_t index)
{
    ...
    if (!check_run_result(test_case, octotiger::radiation_cpu_kernel))
    {
    ...
    if (!check_run_result(test_case, octotiger::radiation_v2_kernel))
    {
        std::printf("case %zd code integrity check failed.\n", index);
        return false;
    }
    ...
    if (!check_run_result(test_case, octotiger::radiation_gpu_kernel))
    {
        std::printf("case %zd code integrity check failed.\n", index);
        return false;
    }
